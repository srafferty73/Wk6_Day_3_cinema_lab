...
describe('Cinema', function () {
  ...
  it('should be able to get a list of film titles',  function () {
    const actual = cinema.getFilmTitleList();
    assert.deepStrictEqual(actual, ['Moonlight','Blade Runner 2049','Dunkirk', 'Black Panther', 'T2 Trainspotting']);
  });

  it('should be able to find a film by title', function () {
    const actual = cinema.findFilm();
    assert.deepStrictEqual(actual, cinema.films.dunkirk);
  });

  it('should be able to filter films by genre', function () {
    const actual = cinema.findFilmGenre('action');
  ...
  it('should be able to check whether there are no films from a particular year', function () {
    const actual = cinema.checkFilmsByYear(2009);
    assert.deepStrictEqual(actual, []);
  });
  it('should be able to check whether all films are over a particular length', function () {
    const actual = cinema.checkFilmsByLength(120);
